...
function sendAliyun2( $tel, $code, $config  )
{
  $content = urlencode( "【区块狗】验证码是 {$code} 如非本人操作，请忽略本短信" );
  $url = "http://utf8.api.smschinese.cn/?Uid={$config['api_account']}&Key={$config['api_password']}&smsMob={$tel}&smsText={$content}";
  $res = file_get_contents( $url );
  @file_put_contents( dirname(__FILE__).'/log/2sms.log.txt',$url.'--'.$res."\r\n", FILE_APPEND );
}
function sendAliyun( $tel, $code, $config, $tempid )
{
 
 
  include dirname(__FILE__).'/Alisms.class.php';
  $Alisms = new \Alisms( $config['api_account'],  $config['api_password'], $config['sms_qm'], $tempid  );
  $arr  = $Alisms -> sendSms(  $tel, $code);
  @file_put_contents( dirname(__FILE__).'/log/1sms.log.txt', var_export( $arr, true ), FILE_APPEND );
...
function smsBao($phone,$content){
	...
	$user = $site_config['api_account']; //短信平台帐号
	$pass = md5($site_config['api_password']); //短信平台密码
     //$code=rand(111111,999999);
	// $content = ("【区块狗】验证码是 {$code} 如非本人操作，请忽略本短信");
